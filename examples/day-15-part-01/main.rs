...
fn main() {
    // let file = File::open("./examples/day-15-part-01/input-test.txt").expect("file not found");
    let file = File::open("./examples/day-15-part-01/input.txt").expect("file not found");
    ...
    for (i, line) in buff.lines().enumerate() {
        ...
        for (j, c) in line.chars().enumerate() {
            match (&state_parse, c) {
                (StateParse::Map, WALL | BOX) => row.push(Some(c)),
                ...
                (StateParse::Map, _) => row.push(None),
    ...
    for command in commands.iter() {
        ...
        walk_or_push(mov, &mut pos, &mut matrix);
    }

    print_matrix(&matrix);
    println!("Result -> {}", sum_box_gps(&matrix));
}
fn walk_or_push(mov: Point<i64>, pos: &mut Point<i64>, m: &mut [Vec<Option<char>>]) -> bool {
    ...
    if !walk_or_push(mov, &mut (*pos + mov).clone(), m) {
        return false;
    }
    let next = *pos + mov;
